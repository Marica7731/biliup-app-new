...
export const useUtilsStore = defineStore('template', () => {
    ...
    const fetchCuluaSearch = async (query: string) => {
        ...
        const url = `https://www.culua.com/api/search?${params.toString()}`
        ...
        try {
            ...
            if (!browserResponse.ok) {
                throw new Error(`HTTP ${browserResponse.status}`)
            }

            return await browserResponse.json()
        } catch (browserError) {
            console.warn('浏览器 fetch culua 搜索失败，回退到 plugin-http:', browserError)
            ...
            try {
                ...
                console.error('获取 culua 搜索结果失败:', pluginError)
                throw pluginError
            }
        }
    }
